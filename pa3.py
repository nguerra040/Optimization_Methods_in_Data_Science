...
# -*- coding: utf-8 -*-
'''
Edited by: Nicolas Guerra
Due Date: 5/6/2022
Edits: I added the gradient computation for logistic regression.
...
class LogisticRegressionProblem(OptProblem):
    ...
    def gradient(self, x):
        # Computing the gradient using broadcasting
        Zx = self._Z_train.dot(x)
        yZx = self._y_train * Zx
        denom = 1 + np.exp(yZx)
        neg_y_denom = -self._y_train / denom
        sum_rows = sum(neg_y_denom[:, np.newaxis]*self._Z_train)
        grad = sum_rows / self._N_train
        if self._regu > 0.:
            # Do not include bias term in regularization
            grad += 2 * self._regu * np.append(sum_rows[:-1], 0)
        return grad
